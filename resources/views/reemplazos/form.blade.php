<x-app-layout><x-slot name="header"><div class="flex items-center justify-between"><div><h2 class="text-xl font-semibold">{{ $tramite ? $tramite->codigo : 'Nueva Solicitud de Reemplazo' }}</h2><p class="text-sm text-gray-500">Estado: {{ $tramite?->estadoTramite?->nombre ?? 'Borrador' }} @if($tramite) · {{ $tramite->unidadOrganizacional->nombre }} @endif</p></div><div class="flex gap-3"><button form="reemplazo-form" class="rounded bg-indigo-700 px-4 py-2 text-sm font-semibold text-white">{{ $tramite?->estadoTramite?->codigo === 'DEVUELTA_PARA_CORRECCION' ? 'Guardar cambios' : 'Guardar borrador' }}</button>@if($tramite)<form method="POST" action="{{ route('reemplazos.send',$tramite) }}">@csrf<button class="rounded bg-emerald-700 px-4 py-2 text-sm font-semibold text-white">{{ $tramite->estadoTramite->codigo === 'DEVUELTA_PARA_CORRECCION' ? 'Reenviar a Gestión de Personas' : 'Enviar a Gestión de Personas' }}</button></form>@endif</div></div></x-slot>
...
>
    ...
    <label class="block">Tipo de reemplazo<select name="tipo_reemplazo_id" class="mt-1 w-full rounded border-gray-300"><option value="">Pendiente</option>@foreach($tipos as $tipo)<option value="{{ $tipo->id }}" @selected(old('tipo_reemplazo_id',$detalle?->tipo_reemplazo_id)==$tipo->id)>{{ $tipo->nombre }}</option>@endforeach</select></label><h4 class="font-medium">Período del funcionario reemplazado</h4><div class="grid grid-cols-2 gap-3"><label>Desde<x-text-input type="date" class="mt-1 w-full" name="fecha_funcionario_desde" x-model="fechaFuncionario" x-on:change="if (unidadId) cargarFuncionarios()" value="{{ $fechaFuncionarioInicial }}"/></label><label>Hasta<x-text-input type="date" class="mt-1 w-full" name="fecha_funcionario_hasta" value="{{ old('fecha_funcionario_hasta',$detalle?->fecha_funcionario_hasta?->toDateString()) }}"/></label></div>
</section>
<section class="space-y-4 rounded-xl bg-white p-6 shadow"><h3 class="font-semibold">Reemplazante único</h3><label class="block">Persona existente<select name="reemplazante_id" class="mt-1 w-full rounded border-gray-300"><option value="">Pendiente / registrar nueva</option>@foreach($personas as $persona)<option value="{{ $persona->id }}" @selected(old('reemplazante_id',$detalle?->reemplazante_id)==$persona->id)>{{ $persona->nombre_completo }} · {{ $persona->rut }}</option>@endforeach</select></label><details class="rounded border p-3"><summary class="cursor-pointer text-sm text-indigo-700">+ Registrar nuevo reemplazante</summary><div class="mt-3 grid gap-3"><x-text-input name="nuevo_reemplazante_rut" value="{{ old('nuevo_reemplazante_rut') }}" placeholder="RUT"/><x-text-input name="nuevo_reemplazante_nombres" value="{{ old('nuevo_reemplazante_nombres') }}" placeholder="Nombres"/><div class="grid grid-cols-2 gap-2"><x-text-input name="nuevo_reemplazante_apellido_paterno" value="{{ old('nuevo_reemplazante_apellido_paterno') }}" placeholder="Apellido paterno"/><x-text-input name="nuevo_reemplazante_apellido_materno" value="{{ old('nuevo_reemplazante_apellido_materno') }}" placeholder="Apellido materno"/></div></div></details><h4 class="font-medium">Período efectivo del reemplazante</h4><div class="grid grid-cols-2 gap-3"><label>Desde<x-text-input type="date" class="mt-1 w-full" name="fecha_reemplazante_desde" value="{{ old('fecha_reemplazante_desde',$detalle?->fecha_reemplazante_desde?->toDateString()) }}"/></label><label>Hasta<x-text-input type="date" class="mt-1 w-full" name="fecha_reemplazante_hasta" value="{{ old('fecha_reemplazante_hasta',$detalle?->fecha_reemplazante_hasta?->toDateString()) }}"/></label></div>@if($detalle && $detalle->diasFuncionario() > 0)<div class="rounded bg-slate-50 p-3 text-sm"><p>Período funcionario: {{ $detalle->diasFuncionario() }} días calendario</p><p>Período reemplazante: {{ $detalle->diasReemplazante() }} días calendario</p><p>Días sin cobertura: {{ $detalle->diasSinCobertura() }}</p>@if($detalle->coberturaParcial())<p class="mt-2 rounded bg-amber-50 p-2 text-amber-800">El reemplazante cubrirá {{ $detalle->diasReemplazante() }} de los {{ $detalle->diasFuncionario() }} días del período solicitado. Quedarán {{ $detalle->diasSinCobertura() }} días sin cobertura.</p>@endif</div>@endif</section>
<section class="rounded-xl bg-white p-6 shadow"><h3 class="mb-3 font-semibold">Justificación</h3><textarea name="justificacion" rows="8" class="w-full rounded border-gray-300" placeholder="Puede completarse progresivamente">{{ old('justificacion',$detalle?->justificacion) }}</textarea></section>
<section class="rounded-xl bg-white p-6 shadow"><h3 class="mb-3 font-semibold">Documentos</h3>@if(!$tramite)<p class="text-sm text-gray-500">Guarde primero el borrador para habilitar adjuntos.</p>@else @can('tramites.adjuntos.cargar')<div class="space-y-3"><input form="adjunto-form" type="file" name="archivo" required><select form="adjunto-form" name="tipo_documento_id" class="w-full rounded border-gray-300"><option value="">Sin tipo documental</option>@foreach($tiposDocumento as $tipo)<option value="{{ $tipo->id }}">{{ $tipo->nombre }}</option>@endforeach</select><x-secondary-button form="adjunto-form">Adjuntar</x-secondary-button></div>@endcan<div class="mt-4 space-y-2">@forelse($tramite->adjuntos as $adjunto)<p class="text-sm">{{ $adjunto->original_name }} · v{{ $adjunto->version }} · {{ $adjunto->status }}</p>@empty<p class="text-sm text-gray-500">Sin documentos adjuntos.</p>@endforelse</div>@endif</section>
