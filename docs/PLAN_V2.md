...
El catálogo mínimo `tipos_reemplazo` vuelve como catálogo configurable y desactivable; al cierre de V2A permanecía sin valores y su carga inicial se aborda en V2B. La relación con Personas no crea duplicados ni alta automática en dotación. Formularios, workflow, revisión de Gestión de Personas, snapshots documentales, PDF, DocDigital y formalización permanecen pendientes; el código PDF histórico desconectado no fue adaptado ni eliminado. La suite completa de V2A ejecuta 92 pruebas y 468 aserciones.

## Reemplazos V2B implementado: creación y edición de borradores

La creación de una Solicitud de Reemplazo exige el permiso funcional `reemplazos.crear` y acceso operativo vigente a la unidad seleccionada. Las unidades provienen exclusivamente de `AccesoOperativoService`; el funcionario se valida contra la dotación efectiva de esa unidad en la fecha inicial. El trámite y su único detalle se crean transaccionalmente con ULID, código correlativo, tipo `REEMPLAZO`, estado `BORRADOR` e historial. El creador u otro usuario autorizado por permiso y alcance puede editar mientras el trámite permanezca en dicho estado, incluso si cambia de unidad, caso en que se revalida la dotación.

El borrador admite datos progresivos: reemplazante, su periodo, tipo, justificación y documentos pueden quedar pendientes. Los periodos informados siempre deben ser internamente consistentes; la superposición entre borradores completos del mismo funcionario se rechaza. Puede seleccionarse una Persona existente o registrar otra mediante el flujo común de Persona y RUT único, sin crear usuario, acceso ni vínculo de dotación. El formulario usa tarjetas, distingue los dos periodos y muestra días inclusivos y advertencia informativa cuando la cobertura es parcial, sin ofrecer múltiples reemplazantes.
...
Los formularios solo ofrecen estamentos, profesiones y calidades contractuales activas. Si falta un catálogo obligatorio, la interfaz bloquea la acción con una explicación administrativa. Las calidades contractuales oficiales se resuelven por código estable y `REEMPLAZO` queda preseleccionada, pero editable, durante la formalización. No se siembran otros valores inventados. Regeneración, desformalización, firma/visación, integración DocDigital, múltiples reemplazantes y Horas Extraordinarias continúan fuera de alcance.
