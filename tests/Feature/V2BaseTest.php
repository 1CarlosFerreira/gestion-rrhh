...
class V2BaseTest extends TestCase
{
    ...
    {
        foreach (['users', 'personas', 'roles', 'permissions', 'tramites', 'tramite_historial', 'tramite_adjuntos', 'documento_plantillas', 'documentos_generados'] as $table) {
            $this->assertTrue(Schema::hasTable($table), "Falta la tabla transversal {$table}.");
        }

        foreach (['unidades_servicios', 'persona_unidad_vinculos', 'user_unidades', 'grados_eus', 'tramite_reemplazos', 'tramite_horas_extra', 'docdigital_registros'] as $table) {
